...
Func coordinati_privazki()
	...
	MsgBox(0, "Внимание!!!", "Мы нашли точку привязки с параметром графики " & $grafika & " по координатам Х=" & $tx - $bx & " Y=" & $ty - $by & @CR & @CR & "Мы определили координаты лагеря как X=" & $ax & " Y=" & $ay & @CR & @CR & "Для наглядности поводили мышкой между ними и скопировали параметры в буфер обмена в виде 'x_цели,y_цели,точка_привязки.bmp,x_куда_центровать,y_куда_центровать' ")
	ClipPut($ax & "," & $ay & "," & GUICtrlRead($file_gui) & "," & $tx - $bx & "," & $ty - $by)
	Return 1
EndFunc
